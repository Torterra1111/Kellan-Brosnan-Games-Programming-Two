...
Audio::Audio() //Constructer
{
	device = alcOpenDevice(NULL); //open sound card
	if (device == NULL) //if there is no device
	{
	...
	if (context == NULL) //If no ALcontext
	{
...
unsigned int Audio::loadSound(const char* filename) //Loading the sound from file
{
	bool found = false;
...
void Audio::deleteSound(unsigned int id) //byebye sound
{}

void Audio::playSound(unsigned int id) //play sound
{
	alSourcePlay(id);
}
